...
import { resolvePrefs } from '../lib/dashboardPrefs.js';
...
export default function Dashboard({ session }) {
  ...
  // ── Derived values ────────────────────────────────────────────
  const prefs     = resolvePrefs(profile?.dashboard_prefs);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }} className="fade-in">
      ...
      {/* ── Agent Briefing ─────────────────────────────────── */}
      {prefs.agentBriefing && interventions.length > 0 && (
        <AgentBriefing
      ...
      {(prefs.deadlineCards || prefs.focusTimer) && (
        <div style={{
          display: 'grid',
          gridTemplateColumns:
            prefs.deadlineCards && prefs.focusTimer
              ? (todayT.length > 0 && tomorrowT.length > 0 ? '1fr 1fr 1fr' : todayT.length > 0 || tomorrowT.length > 0 ? '1fr 1fr' : '1fr')
              : prefs.deadlineCards
                ? (todayT.length > 0 && tomorrowT.length > 0 ? '1fr 1fr' : '1fr')
                : '1fr',
          gap: 14,
        }}>
          {prefs.deadlineCards && todayT.length > 0 && (
          ...
          {prefs.deadlineCards && tomorrowT.length > 0 && (
            <div className="card">
          ...
          {prefs.focusTimer && (
            <FocusTimer compact={
              !prefs.deadlineCards || (todayT.length === 0 && tomorrowT.length === 0)
            } />
          )}
        </div>
      )}
      ...
      {/* ── AI Recommendation ──────────────────────────────── */}
      {prefs.aiRecommendation && recommendation && (
...
/* ── Timer alert helpers (outside component — no closure issues) ──
   playBeep receives the already-unlocked AudioContext from the ref.
   All three are called from the done useEffect, never from inside
   a setState updater, so they are safe side effects.
──────────────────────────────────────────────────────────────────── */
function playBeep(ctx) {
  ...
  try {
    ...
    beep(432, t,        0.18);  // Beep 1: 432 Hz
    beep(523, t + 0.30, 0.18);  // Beep 2: C5, 120 ms pause between
  } catch { /* silent fail */ }
}
...
function sendNotification() {
  ...
  try {
    new Notification('Task Catalyst', {
      body: 'Focus session complete — take a break! 🎉',
      icon: '/favicon.svg',
    });
  } catch { /* Safari/FF quirks */ }
}

function flashTab(tabFlashRef) {
  // In-app guaranteed visual: change document title 7×
  const titles = ['✅ Done! — Task Catalyst', 'Task Catalyst'];
  let i = 0;
...
function FocusTimer({ compact }) {
  ...
  const audioCtxRef   = useRef(null);   // created on first Start (user gesture)
  const endTimeRef    = useRef(null);   // absolute ms when timer should hit 0
  const tickRef       = useRef(null);   // rAF/interval handle
  const tabFlashRef   = useRef(null);
  const notifAsked    = useRef(false);

  // ── Tick: use absolute end-time so tab sleep can't drift ───
  // We schedule the next tick 250 ms into the future (smooth arc),
  // but always compute secondsLeft from wall-clock, not by counting ticks.
  useEffect(() => {
    if (!running) {
    ...
    function tick() {
      ...
      if (remaining <= 0) {
        setRunning(false);
        setDone(true);
        // don't call fireAlerts here — it'll be called by the done useEffect below
      } else {
        tickRef.current = setTimeout(tick, 250); // 250 ms for smooth arc, not drift
      }
    }
    tick();
  ...
  // ── Fire alerts exactly once when done flips to true ───────
  useEffect(() => {
    if (!done) return;
    playBeep(audioCtxRef.current);
    sendNotification();
    flashTab(tabFlashRef);
  }, [done]);
  ...
  // ── handleStart: create AudioContext here (user gesture) ───
  function handleStart() {
    if (done) return;

    // Request notification permission on first Start
    if (!notifAsked.current && 'Notification' in window && Notification.permission === 'default') {
      notifAsked.current = true;
      Notification.requestPermission();
    }

    if (!running) {
      // Create/resume AudioContext inside user gesture so browsers allow it
      ...
      // Set absolute end time: now + remaining seconds
  ...
  function reset() {
    setRunning(false);
    setDone(false);
    const s = PRESETS[preset].seconds;
    setSecs(s);
  ...
  const secs_d   = String(secondsLeft % 60).padStart(2, '0');
  const R        = 28;
    ...
    return (
      <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 18px' }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
          <button className="btn btn-primary" onClick={handleStart} disabled={done}
